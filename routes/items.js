...
router.get('/new', catchErrors(async(req, res, next) => {
  ...
  res.render('items/index_new', {items: items, query: req.query});
}));

router.get('/price', catchErrors(async(req, res, next) => {
  const page = parseInt(req.query.page) || 1;
  const limit = parseInt(req.query.limit) || 10;
  ...
  res.render('items/index_price', {items: items, query: req.query});
}));
